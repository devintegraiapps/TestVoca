...
export default function App() {
  ...
  function onFinish() {
    if (!allAnswered) { alert("Responda todas as afirmações para ver o resultado."); return; }
    setStep("result");
  }
  function restart() { setAnswers({}); setStep("quiz"); }
  ...
  function runTests() {
    ...
    const t3Ok = t3Text.includes("Meu resultado no Teste Vocacional (RIASEC):") && t3Text.includes("Top 5 cursos sugeridos:") && t3Text.split("\n").length > 6;
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="mb-4 rounded-xl border border-slate-700 bg-slate-800 p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <button onClick={runTests} className="px-3 py-1 rounded-lg bg-indigo-500 text-slate-900 font-bold hover:bg-indigo-400 text-sm">Rodar testes</button>
          </div>
        ...
        {!consent ? (
          <div className="bg-slate-800 rounded-2xl p-5 border border-slate-700">
            ...
            <p className="text-slate-300 mb-4">Este protótipo usa suas respostas apenas para cálculo local do resultado. Não há coleta de dados. Ao continuar, você concorda com este uso para fins de demonstração.</p>
            <button onClick={() => setConsent(true)} className="px-4 py-2 rounded-xl bg-emerald-500 font-bold text-slate-900 hover:bg-emerald-400">Concordo e começar</button>
          </div>
        ) : step === "quiz" ? (
          <div className="bg-slate-800 rounded-2xl p-5 border border-slate-700">
              ...
              {QUESTIONS.map((q) => (
                <div key={q.id} className="p-4 rounded-xl bg-slate-900 border border-slate-700">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {[0,1,2,3,4].map((v) => (
                      <button key={v} onClick={() => setAnswer(q.id, v)} className={"px-3 py-1 rounded-lg border text-sm " + ((answers[q.id] ?? -1) === v ? "bg-blue-500 border-blue-400 text-white" : "bg-slate-800 border-slate-600 text-slate-200 hover:bg-slate-700")} aria-pressed={(answers[q.id] ?? -1) === v} aria-label={`Marcar ${v} para a questão ${q.id}`}>
                        {v}
                      </button>
                    ))}
                  </div>
            ...
            <div className="mt-5 flex items-center justify-between">
              ...
              <button onClick={onFinish} className={"px-4 py-2 rounded-xl font-bold " + (Object.keys(answers).length === QUESTIONS.length ? "bg-emerald-500 text-slate-900 hover:bg-emerald-400" : "bg-slate-700 text-slate-300 cursor-not-allowed")}>Ver resultado</button>
            </div>
          </div>
        ) : (
          <div className="bg-slate-800 rounded-2xl p-5 border border-slate-700">
              ...
              {recommended.map((rc) => (
                <div key={rc.name} className="rounded-xl border border-slate-700 bg-slate-900 p-3">
                  ...
                  <ul className="text-xs text-slate-300 list-disc pl-4 space-y-1">
                    {rc.reasons.map((r, i) => (<li key={i}>{r}</li>))}
                  </ul>
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button onClick={restart} className="px-4 py-2 rounded-xl border border-slate-600 text-slate-100 hover:bg-slate-700">Refazer teste</button>
              <button onClick={() => {
              ...
              }} className="px-4 py-2 rounded-xl bg-blue-500 font-bold text-slate-900 hover:bg-blue-400">Copiar resultado</button>
            </div>
